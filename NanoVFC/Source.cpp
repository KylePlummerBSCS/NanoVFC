...
//After BasicFileOpen there are several other code examples, none of which are accessable to users. 
                                ...
                                if (SUCCEEDED(hr))
                                {
                                    ...
                                    if (SUCCEEDED(hr))
                                    {
                                        ...
                                        if (SUCCEEDED(hr))
                                        {
											...
											//std::string MyString = CW2A(pszFilePath);

											//RXA21DELETE

											std::string *p_filename1 = new std::string(CW2A(pszFilePath));

											mstrClass *p_mstrClass = new mstrClass();

											std::string *p_delFileName = new std::string(*p_filename1);
											p_delFileName->erase(p_delFileName->find("."));
											p_delFileName->append("_DEL.txt");
											std::string *p_keepFileName = new std::string(*p_filename1);
											p_keepFileName->erase(p_keepFileName->find("."));
											p_keepFileName->append("_OUT.txt");

											int h1 = p_mstrClass->newHLFile(p_filename1);
...
//Commenting out these other examples which are no longer available in execution.
/*
...
int APIENTRY wWinMain(HINSTANCE, HINSTANCE, PWSTR, int)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        {
            { IDC_BASICFILEOPEN,                       L"Choose File" },
        ...
        taskDialogParams.pszMainInstruction = L"Open VFC HL7 File";
        taskDialogParams.pszWindowTitle = L"NanoVFC2";
        ...
        while (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(hr))
            {
				...
				//These are no longer accessable. -Kyle
